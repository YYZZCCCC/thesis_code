## scatter plot about the relationship between age and Vit D Level 
...
lm_model <- lm(vitmaindcheckresult ~ age, data = DF)
...
ggplot(DF, aes(x = age, y = vitmaindcheckresult)) +
  geom_point(color = "black", alpha = 0.6) +  
  geom_smooth(method = "lm", color = "red", linetype = "dashed", se = FALSE) + 
  annotate("text", x = max(DF$age) * 0.8, 
           y = max(DF$vitmaindcheckresult) * 0.9,
           label = equation, color = "red", size = 5, hjust = 0.5) +
  scale_x_continuous(breaks = seq(floor(min(DF$age)),
                                  ceiling(max(DF$age)),
                                  by = 5)) + 
  labs(x = "Age", y = "Vit D Level", 
       title = "Correlation between Age and Vit D Level") +
  theme_minimal() +
